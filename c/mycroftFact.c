		...
		if(NULL==(void*)w) {
			...
			if(NULL==(void*)f) {
					...
					if(NULL==(void*)w->item) {
						...
						if(0==x) x=(w->item->pred->arity)-(p->arity);
						if(0==x) {
							free(plt);
							w->item->facts=ft;
						} else {
